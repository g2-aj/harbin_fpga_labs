...
typedef struct FcLayers {
	std::vector<FcParams> layerParams;
	int noLayers;
} FcLayers;

void initModel(DataShape &inputShape, FcLayers &fc);
void allocateHostBuffer();
void initInputImage(const pgm_t &input_pgm);
void createDeviceBuffer();
bool init_opencl();
unsigned int runApplication(FcLayers &fcModel);
void innerProductLayer(const std::vector<float> &input, std::vector<float> &output, const FcParams &fp);
void reluLayer(std::vector<float> &input);
void softmaxLayer(std::vector<float> &input, std::vector<float> &output);
...
void populateFcParams(FcParams &fp, const float *w, const float *b) {
	fp.img.resize(fp.noOutputs, std::vector<float>(fp.noInputs, 0));
	fp.bias = std::vector<float>(fp.noOutputs, 0);

	for (int i = 0; i < fp.noOutputs; i++) {
		for (int j = 0; j < fp.noInputs; j++) {
			fp.img[i][j] = w[i*fp.noInputs+j];
		}
		fp.bias[i] = b[i];
	}
}

unsigned int runApplication(FcLayers &fc) {
	cl_int status;
	size_t global_work_size[3];
	size_t local_work_size[3];
	scoped_array<cl_event> kernel_event(4);
	//scoped_array<cl_event> kernel_event(4);
	std::vector<float> ip1In, ip1Out, ip2Out, prob;
	...
	cl_ulong time_ns = getStartEndTime(kernel_event, 4);

	// Perform fclayers + relu + smax computation on the host (does not fit on de1-soc)
	status = clEnqueueReadBuffer(queue, pool2.d_output, CL_TRUE, 0,
		pool2.top_shape.x * pool2.top_shape.y * pool2.top_shape.z * sizeof(DTYPE), pool2.h_output, 0, NULL, NULL);
	checkError(status, "Failed to read pool2 output data from the device");

	ip1In.resize(IP1_NO_INPUTS);
	for (int i = 0; i < IP1_NO_INPUTS; i++) {
		ip1In[i] = pool2.h_output[i];
	}
	innerProductLayer(ip1In, ip1Out, fc.layerParams[0]);
	reluLayer(ip1Out);
	innerProductLayer(ip1Out, ip2Out, fc.layerParams[1]);
...
void initModel(DataShape &inputShape, FcLayers &fc) {
	...
	cout << "CNN model pre populate params\n";
